...
SlideSize=3
SlDF11=ApplySliding(DF=DF11, Sliding=SlideSize)
SlDF12=ApplySliding(DF=DF12, Sliding=SlideSize)
...
#New Array structure -- Acc 0.49 -- 100 Epoch
XVal=array(data=0, dim=c(length(ResX), SlideSize*2, ncol(ResX[[1]])))
for(iRow in 1:dim(XVal)[1]){
  for(xChannel in 1:dim(XVal)[3]){
    XVal[iRow,,xChannel]=ResX[[iRow]][[xChannel]]
  }
}

#New Array structure -- Acc 0.46 -- 100 Epoch
XVal=array(data=0, dim=c(length(ResX), SlideSize*2, ncol(ResX[[1]])))
for(iRow in 1:dim(XVal)[1]){
  for(xChannel in 1:dim(XVal)[3]){
    XVal[iRow,xChannel,]=ResX[[iRow]][[xChannel]]
...
#New Array structure -- Acc 0.48 -- 100 Epoch
XVal=array(data=0, dim=c(length(ResX), SlideSize*2, ncol(ResX[[1]])))
for(iRow in 1:dim(XVal)[1]){
  for(xChannel in 1:dim(XVal)[3]){
    XVal[iRow,,xChannel]=t(ResX[[iRow]])[xChannel,]
  }
}


#New Array structure -- Acc 0.61 -- 100 Epoch
XVal=array(data=0, dim=c(length(ResX), SlideSize*2, ncol(ResX[[1]])))

SeqRange=seq(from=1, to=5412, by=6)
RangeDF=data.frame('From'=SeqRange[1:length(SeqRange)], 'To'=c((SeqRange-1)[2:length((SeqRange-1))],5412))
RangeDF$RFrom=1
RangeDF$RTo=6

TEMP=bind_cols(split(x=1:5412, f=sort(1:5412%%6)))
colnames(TEMP)=1:6
RangeDF=bind_cols(RangeDF, TEMP)

ChannelN=1
while(ChannelN<7){
  Counter=1
  for(iRow in 1:nrow(RangeDF)){
    
    RowArray=RangeDF[iRow,]$From:RangeDF[iRow,]$To
    RowTable=RangeDF[iRow,]$RFrom:RangeDF[iRow,]$RTo
    
    for(TableNum in RangeDF[,c(paste0(ChannelN))][Counter]){
      
      for(i in 1:6){
        #Sys.sleep(0.001)
        #print(paste0('RowArray: ',RowArray[i],' - Channel: ', ChannelN, ' - TableNum: ',TableNum, ' - RowTable: ', RowTable[i]))
        XVal[RowArray[i],,ChannelN]=t(ResX[[TableNum]])[RowTable[i],]
      }
      if(any(c(902, 1804, 2706, 3608, 4510, 5412) %in% TableNum)){
        ChannelN=ChannelN+1
      }
